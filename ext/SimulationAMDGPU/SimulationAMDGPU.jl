...
function Simulation._init_fields(
    ...
    AMDGPU.@roc groupsize=threads gridsize=blocks _init_fields_kernel!(
        u, v, roc_offsets, roc_sizes, minL, maxL)
    AMDGPU.synchronize()
...
function Simulation.iterate!(fields::Fields{T, N, <:AMDGPU.ROCArray{T, N}},
...
function _calculate!(fields::Fields{T, N, <:AMDGPU.ROCArray{T, N}},
    ...
    AMDGPU.@roc groupsize=threads gridsize=blocks _calculate_kernel!(
        fields.u, fields.v, fields.u_temp, fields.v_temp, roc_sizes,
        Du, Dv, F, K, noise, dt)
    AMDGPU.synchronize()
end

function Simulation.get_fields(fields::Fields{
        T, N, <:AMDGPU.ROCArray{T, N}}) where {T, N}
    u = Array(fields.u)
