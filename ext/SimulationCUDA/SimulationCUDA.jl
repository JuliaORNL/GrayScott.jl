...
function Simulation._init_fields(
    ...
    println("Initializing fields for CUDA back end")
    ...
    CUDA.@cuda threads=threads blocks=blocks _init_fields_kernel!(
        u, v, cu_offsets, cu_sizes, minL, maxL)
    CUDA.synchronize()
...
function Simulation.iterate!(fields::Fields{T, N, <:CUDA.CuArray{T, N}},
...
function _calculate!(fields::Fields{T, N, <:CUDA.CuArray{T, N}},
    ...
    function _calculate_kernel!(
            u, v, u_temp, v_temp, sizes, Du, Dv, F, K, noise, dt)
    ...
    CUDA.@cuda threads=threads blocks=blocks _calculate_kernel!(
        fields.u, fields.v, fields.u_temp, fields.v_temp, cu_sizes,
        Du, Dv, F, K, noise, dt)
    CUDA.synchronize()
end

function Simulation.get_fields(fields::Fields{
        T, N, <:CUDA.CuArray{T, N}}) where {T, N}
    u = Array(fields.u)
